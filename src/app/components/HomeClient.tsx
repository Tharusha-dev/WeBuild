...
//bg-white/5
  return (
    <div className="min-h-screen bg-[#49409e] text-white font-sans selection:bg-black selection:text-[#6E65C7] overflow-x-hidden">
      ...
      {/* Navigation */}
      <nav className="relative z-10 w-full border-b border-white/20 px-6 py-6 flex justify-between items-center bg-[#49409e]">
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-white animate-pulse" />
          <span className="text-xl tracking-widest uppercase font-bold">
            WeBuild.lk
          </span>
        </div>
        <button
      ...
      <main className="relative z-10">
        <section className="px-6 pt-24 pb-32 max-w-7xl mx-auto">
          {/* SEO UPDATE: Combined H1s into one semantic tag. Visuals are identical. */}
          <h1 className="flex flex-col gap-2 font-bold tracking-tighter text-[11vw] leading-[0.85]">
          ...
          <div className="flex whitespace-nowrap animate-scroll w-max gap-25">
            {[
              ...
              <Image
                ...
                alt={`${tech} automation tool`} // SEO UPDATE: Added description to alt
                width={100}
                height={40}
                className="mx-6 opacity-80 h-[2rem] w-auto"
              />
            ))}
          </div>
        </div>
        ...
        {/* Modules Grid */}
        <section className="max-w-7xl mx-auto px-6 py-32 bg-[#49409e]">
          {/* SEO UPDATE: Added hidden H2 for semantic structure */}
          <h2 className="sr-only">Our Automation Services</h2>
          
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 bg-white/5 gap-px border border-white/20 grid-borders transition-all duration-300">
            {serviceModules.map((mod, index) => (
              <div
                key={mod.code}
                onMouseEnter={() => setHoveredModule(index)}
                onMouseLeave={() => setHoveredModule(null)}
                className="group relative bg-white/5 p-6 h-64 flex flex-col justify-between transition-all duration-200 hover:bg-white hover:text-[#6E65C7] overflow-hidden cursor-crosshair"
              >
                <div>
                  ...
                  <h3 className="text-xl md:text-2xl font-bold leading-tight w-full">
                    {mod.title}
                  </h3>
                </div>
        ...
        <footer className="border-t border-white/20 bg-black/10">
          <div className="max-w-7xl mx-auto px-6 py-20 flex flex-col md:flex-row items-center justify-between gap-10">
            ...
            <div className="flex flex-col items-start gap-6">
              ...
              <a
                href="tel:+94728103079"
                className="flex items-center gap-4 text-xl font-bold tracking-tight hover:opacity-80 transition-opacity"
              >
          ...
          <div className="relative w-full max-w-lg bg-[#6E65C7] border-2 border-white shadow-[8px_8px_0px_0px_rgba(255,255,255,0.3)] p-8 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-between items-start mb-8 border-b border-white/20 pb-4">
              <div>
                ...
                <h2 className="text-2xl md:text-3xl font-bold uppercase tracking-tight mt-4">
                  Book a Call
                </h2>
              </div>
              <button
                onClick={() => setIsModalOpen(false)}
                className="hover:rotate-90 transition-transform p-1 hover:bg-white hover:text-[#6E65C7]"
            ...
            {formStatus === "SUCCESS" ? (
              ...
              <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
                <div className="flex flex-col gap-2">
                  <label className="font-mono text-xs uppercase opacity-80 tracking-widest">
                    Phone Number
                  </label>
                  <input
                    type="tel"
                ...
                {formStatus === "ERROR" && (
                  <p className="text-red-300 font-mono text-xs">
                    Error: Connection failed. Please try again or call us
                    directly.
                  </p>
                )}
                ...
                >
                  <span className="relative z-10 flex justify-between items-center uppercase">
                    {formStatus === "LOADING"
                      ? "TRANSMITTING..."
                      : "Confirm Request"}
                    <span className="font-mono text-xs opacity-50 hidden sm:inline-block">
                      / ENTER ↵
                    </span>
                  </span>
                </button>
